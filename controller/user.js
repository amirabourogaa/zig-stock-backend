...
router.post("/create-user", upload.single("file"), async (req, res, next) => {
  try {
    ...
    const activationUrl = `https://zig-stock-front.vercel.app/activation/${activationToken}`;
...
const createActivationToken = (user) => {
  ...
  return jwt.sign({name: user.name, email: user.email, password: user.password, avatar: user.avatar }, process.env.ACTIVATION_SECRET, {
    expiresIn: "1h",
  });
};
...
router.post(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      console.log("all", { name, email, password, avatar } )
